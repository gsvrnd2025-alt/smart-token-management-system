...
void handleEmbeddedDashboard() {
  ...
  html += "<script>async function scanBT(){document.getElementById('bt-list').innerHTML='<div style=\"text-align:center;padding:15px;color:#94a3b8;\">Scanning nearby Bluetooth devices...</div>';try{let r=await fetch('/api/printer/scan');let d=await r.json();let html='';if(!d||d.length===0){html='<div style=\"text-align:center;color:#94a3b8;padding:15px;\">No nearby Bluetooth devices found.</div>';}else{d.forEach(dev=>{let safeId=dev.address.replace(/:/g,'');html+=`<div class=\"dev\"><div><div class=\"dev-name\">${dev.name||'Bluetooth Device'}</div><div class=\"dev-mac\">${dev.address} (${dev.rssi} dBm)</div></div><button class=\"btn-sm\" id=\"btn-${safeId}\" onclick=\"connectBT('${dev.address}',this)\">Connect</button></div>`;});}document.getElementById('bt-list').innerHTML=html;}catch(e){document.getElementById('bt-list').innerHTML='Scan failed: '+e.message;}}async function connectBT(mac,btn){if(btn){btn.disabled=true;btn.textContent='Connecting...';btn.style.background='#eab308';}try{let r=await fetch('/api/printer/connect?mac='+encodeURIComponent(mac),{method:'POST'});let res=await r.json();if(res.success){if(btn){btn.textContent='Connected ✓';btn.style.background='#22c55e';}alert('Connected to printer: '+mac);}else{if(btn){btn.disabled=false;btn.textContent='Retry';btn.style.background='#ef4444';}alert('Connection failed: '+(res.error||'Check printer power'));}}catch(e){if(btn){btn.disabled=false;btn.textContent='Retry';btn.style.background='#ef4444';}alert('Request error: '+e.message);}}</script></body></html>";
  server.send(200, "text/html", html);
}
